...
The retrieval knowledge base is grounded in statutory Indian financial and data privacy frameworks, ingested via `scripts/ingest_docs.py`:

- **Document Corpus**: 5 sample regulatory PDF corpora stored in `data/`:
  - `sebi_lodr_2015.pdf`: SEBI (Listing Obligations and Disclosure Requirements) Regulations, 2015
  - `sebi_sast_2011.pdf`: SEBI (Substantial Acquisition of Shares and Takeovers) Regulations, 2011
  - `sebi_icdr_2018.pdf`: SEBI (Issue of Capital and Disclosure Requirements) Regulations, 2018
  - `rbi_model_risk_2023.pdf`: RBI Guidelines on Model Risk Management for Credit and Liquidity
  - `dpdpa_2023.pdf`: Digital Personal Data Protection Act, 2023
- **Chunking Pipeline**:
  - `chunk_size = 512` characters, `chunk_overlap = 64` characters.
  - Paragraph-first semantic boundaries with word-safe boundary overlap (`_word_safe_tail`).
  - Strict minimum chunk length filter (`len(chunk) >= 20` characters).
- **Dense Vector Embeddings**: `fastembed` using `sentence-transformers/all-MiniLM-L6-v2` (384-dimensional dense vectors, normalized for cosine similarity, batched at 64 items).
- **Vector Storage**: Qdrant collection `sebi_rbi_docs` indexed with HNSW.
- **Corpus Nature**: Demonstration PDFs generated by `scripts/create_sample_docs.py` to ensure zero copyright barriers for testing. Real full-text gazette PDFs can be dropped into `data/` and re-indexed with a single command.
...
### Known Limitations
- **Corpus Scope**: Currently indexed with 5 representative sample documents rather than the full legislative gazette of SEBI Master Circulars and RBI Master Directions.
- **Text-Only PDF Extraction**: PyMuPDF extraction (`fitz.get_text()`) extracts plain-text blocks; tabular disclosure matrices and schedules are not parsed structurally.
- **Groq Free-Tier Rate Limits**: Production inference operates under Groq cloud limits (~30 requests/minute), requiring rate limiting for bulk queries.
- **Volatile BM25 Index**: The BM25 index is maintained in-memory across the worker lifecycle; while rebuilt dynamically from Qdrant scroll on startup, it is not persisted to disk.
- **Collection Overwrite on Ingest**: Running `scripts/ingest_docs.py` calls `recreate_collection`, wiping previous chunks without incremental document versioning.
- **Fixed Fusion Weights**: The hybrid retrieval scoring weight (60% dense vector, 40% BM25 sparse) is hand-tuned rather than empirically optimized via automated grid search over a golden dataset.

### Roadmap
- [ ] **Full Gazette Ingestion**: Index comprehensive SEBI Master Circulars, PIT (Prohibition of Insider Trading), and AIF regulations.
- [ ] **Table-Aware Extraction**: Integrate Camelot/Tabula or vision-based document parsing for regulatory financial tables and disclosure schedules.
- [ ] **Incremental Document Versioning**: Add document hash and timestamp metadata to support append-only updates without full index reconstruction.
- [ ] **Reranker Layer**: Introduce a cross-encoder reranker (e.g. `bge-reranker-large`) on top of top-K hybrid candidates.
- [ ] **Session & Thread History**: Implement persistent multi-turn chat history with Postgres/Redis session storage.
